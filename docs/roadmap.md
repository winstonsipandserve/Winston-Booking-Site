...
4. **Guest fee ₱100 and the guest cap.** Seed value; the wizard's guest-count maximum (7 for a member whose term covers the slot, 3 otherwise); validation in `POST /api/bookings` and `PATCH /api/bookings/[id]` — the anonymous path is always capped at 3, since it never grants member benefits.
