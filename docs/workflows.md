...
3. **Single-phase pricing.** `POST /api/bookings` resolves the membership **covering the slot start** itself (`getMembershipActiveAt`) and reads that term's tier from `MEMBERSHIP_TIER_PLANS`: the tier's `bookingDiscountPercent` comes off the base court/simulator rate (rounded half-up in centavos, never off the guest fee or coaching) and is snapshotted as `Booking.memberDiscountCentavos`; the tier's `advanceBookingDays` bounds how far ahead the slot may be (`manilaCalendarDaysBetween(now, slotStart)`), falling back to `NON_MEMBER_ADVANCE_BOOKING_DAYS` when no term covers the slot. `customerId` is attached directly at hold creation. There is no PATCH round-trip.
   - **Perks.** Unless the request declines them (`useGuestPasses` / `useBirthdayPerk: false`), guest passes cover up to the term's remaining passes and the birthday hour applies to an eligible slot (`src/lib/member-perks.ts`). Both allowances are **derived**, never counted: passes used = sum of `guestPassesUsed` over the member's slot-occupying bookings inside the term (confirmed, or a hold inside its window — `occupyingSlotCondition`), and the birthday hour is used when such a booking has `birthdayPerkApplied`. They are priced from a first read, then re-checked inside the transaction after the per-client advisory lock; a lost race answers `409`. An abandoned hold therefore releases its perks with no bookkeeping.
...
The guest-count control and the ₱100-per-guest fee count **non-member guests only** and apply on both paths; the cap is 7 non-member guests when a term covers the slot and 3 otherwise, and the anonymous path is always held to 3 (`maxGuestsForRateTier` in `src/lib/booking-limits.ts`, checked in `POST /api/bookings` before any write). A guest who is themself a Winston member is free and is never entered into this control — the booking system has no record of them; see Member check-in below and [business.md](business.md) → Guest Fee.
...
5. **Membership branch** — pick the start date (payment time, or one millisecond after the customer's latest unexpired term for an early renewal), compute the end date from the tier as end-of-day Manila, create the `Membership` row (tier and `isFounding` copied from the payment row, credit balance 0 — no grant), generate an activation token, and send the activation (or renewal) email plus a staff notification. The amount and Founding flag were fixed when the payment row was priced (see Membership: application → approval → activation, step 5).
...
1. **Stage and submit.** The applicant completes the form at `/membership/apply` with name, address, contact number, email, and three government ID images. The app obtains three unique, non-overwritable signed URLs from the server and the browser uploads directly to private Storage, avoiding Vercel's 4.5 MB function-body limit. Storage enforces JPEG/PNG and 5 MB per object. The final application request contains only the upload-session reference; the server fetches every staged object, fully decodes it as its declared type, limits it to 25 million pixels, strips metadata, re-encodes it, and writes only the sanitized image to the application path. The temporary objects are deleted on finalization, upload failure, or the daily expiry cron after three hours. A customer record is resolved or created. Staff receive a notification email.
...
2. **Review.** An admin opens the application, views the ID images through signed URLs in an admin-only lightbox, and approves or rejects. Rejection requires a non-empty reason. Either way an activity log row is written in the same transaction as the mutation. The decision email (payment link or rejection) is sent after the transaction; its real delivery result comes back as `paymentEmailSent` / `rejectionEmailSent`, and the admin sees an error toast rather than a success message when Resend rejects or the send throws. The decision itself stands either way — a failed payment-link email is retried with Resend Payment Link.
...
5. **Payment.** The applicant opens `/membership/pay/[id]` and pays through PayMongo Checkout. `POST /api/membership-payments` prices the `MembershipPayment` row under the Founding seat lock (`withFoundingSeatLock` + `quoteMembershipPrice` in `src/lib/membership-founding.ts`): Premier is ₱5,000 and `isFounding` when the customer already holds Founding status or fewer than 100 seats are taken, counting paid Founding memberships plus unpaid Founding rows created inside the 48-hour link lifetime. The approval and resend emails quote the same helper without reserving a seat.
6. **Activation.** The webhook's membership branch creates the `Membership` with its tier, Founding flag, and dates, copies the application's date of birth onto the customer (the public form never edits a customer), and issues an activation token. The activation email carries a **PDF membership certificate** (first-time activation only).
...
Either way the new payment row is priced through the same Founding helper (a Founding Member renewing into Premier keeps ₱5,000 without consuming a seat) and the webhook creates a fresh membership with no credit grant. For an early renewal the new row's `startDate` is one millisecond after the current term's `endDate` (i.e. the next Manila day); the renewal email states the start date. The account card shows a queued renewal's end date under the current term. **Renewal never attaches the certificate PDF.**

---

## Credit top-up

- **Self-service** — an active member uses "Top Up Booking Credit" on `/account`, choosing one of four fixed presets (₱1,000 / ₱2,500 / ₱5,000 / ₱10,000), and pays through PayMongo. The webhook's top-up branch credits the balance and emails a confirmation. Shown **only** for an active, unexpired membership. If the payment lands after the term has ended, the credit is still applied and staff get a `Membership | Top-Up After Expiry | …` notification to arrange a refund or renewal (see [business.md](business.md)).
...
This is also how a **guest's** membership is confirmed when they claim free entry as a member-guest on someone else's booking: staff scan or key in the guest's own QR/code at `/admin/check-in`, off the booking record entirely. Nothing about a member-guest is written back to the booking — see [business.md](business.md) → Guest Fee.
