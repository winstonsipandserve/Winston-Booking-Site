...
6. **Activation.** The webhook's membership branch creates the `Membership` with its tier, Founding flag, and dates, and issues an activation token. The activation email carries a **PDF membership certificate** (first-time activation only).
