...
function DisabledIconButton({ label, variant }: { label: string; variant: 'edit' | 'delete' }) {
  return (
    <button
      type="button"
      disabled
    ...
    <div>
      <div className="mb-6 flex items-center justify-between gap-3">
        ...
        <button
          type="button"
          disabled
          className="inline-flex items-center gap-1 rounded-lg border border-gray-300 px-3 py-1.5 text-xs font-medium text-gray-400 disabled:cursor-not-allowed"
        >
          + Add Bulletin
        </button>
      </div>
          ...
          {bulletins.map((bulletin) => (
            ...
            >
              ...
              <div className="flex shrink-0 items-center gap-2">
                <DisabledIconButton label={`Edit ${bulletin.title}`} variant="edit" />
                <DisabledIconButton label={`Delete ${bulletin.title}`} variant="delete" />
              </div>
            </div>
          ))}
        </div>
