            ...
            <Reveal key={category.title} delayMs={categoryIndex * 100}>
              ...
              <div className="mt-5 flex flex-col">
                {category.items.map((item) => (
                  <div
                    key={item.name}
                    className="border-b border-brand-dark/10 py-4 first:pt-0 last:border-b-0 last:pb-0"
                  >
                    <div className="flex items-baseline justify-between gap-3">
                      <p className="font-serif text-base text-brand-dark">{item.name}</p>
                      <p className="shrink-0 text-sm font-medium text-accent-primary">
                        {formatCentavos(item.priceCentavos)}
                      </p>
                    </div>
                    <p className="mt-1 text-sm text-brand-mid">{item.description}</p>
                  </div>
                ))}
              </div>
            </Reveal>
