...
export default function DashboardBookingCalendar({ initialCalendar }: { initialCalendar: BookingCalendarData }) {
  ...
  return (
    <section className="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-800 dark:bg-gray-900" aria-labelledby="booking-calendar-heading">
      ...
      <div className="grid grid-cols-7 gap-x-1.5 gap-y-1" aria-label={`${formatMonth(calendar.month)} confirmed booking activity`}>
        ...
        {cells.map((day, index) => {
          if (day === null) return <div key={`blank-${index}`} className="h-7" aria-hidden="true" />
          ...
          return (
            <time
              ...
              className={`flex h-7 w-full items-center justify-center rounded-md text-xs font-medium tabular-nums transition-colors ${activityClass(count)} ${
