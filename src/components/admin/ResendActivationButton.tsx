...
export default function ResendActivationButton({
  applicationId,
  email,
}: {
...
function ResendActivationModal({
  ...
  async function handleConfirm() {
    ...
    setError(null)
    try {
      const res = await fetch(`/api/admin/memberships/${applicationId}/resend-activation`, {
        method: 'POST',
      })
      ...
      router.refresh()
      setConfirmation('Activation email sent.')
      setIsSubmitting(false)
    } catch {
  ...
  return (
    <Modal isOpen={isOpen} onClose={handleClose} title="Send Activation Email" variant="neutral">
      {isOpen &&
        (confirmation ? (
          ...
          <div className="flex flex-col gap-4">
            <p className="text-sm text-gray-900 dark:text-gray-100">
              This member has never set a password and can&apos;t sign in. Send a new activation
              link to <span className="font-medium">{email}</span>? Any earlier unused link will
              stop working.
            </p>
