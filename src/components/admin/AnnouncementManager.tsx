...
function statusFor(announcement: AdminAnnouncement): string {
  ...
  const now = Date.now()
  if (new Date(announcement.startAt).getTime() > now) return 'Scheduled'
    ...
    <Modal isOpen={isOpen} onClose={onClose} title={announcement ? 'Edit Announcement' : 'Add Announcement'} maxWidthClassName="max-w-2xl" variant="neutral">
      <form onSubmit={submit} className="scrollbar-thin flex max-h-[78vh] flex-col gap-5 overflow-y-auto pr-1">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <label className="flex flex-col gap-1 text-sm text-gray-700 dark:text-gray-300">Starts *<input type="datetime-local" value={startAt} onChange={(event) => setStartAt(event.target.value)} required className="rounded-lg border border-gray-200 bg-white px-3 py-2 text-gray-900 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-100" /></label>
          <label className="flex flex-col gap-1 text-sm text-gray-700 dark:text-gray-300">Ends (optional)<input type="datetime-local" value={endAt} onChange={(event) => setEndAt(event.target.value)} className="rounded-lg border border-gray-200 bg-white px-3 py-2 text-gray-900 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-100" /></label>
        </div>
