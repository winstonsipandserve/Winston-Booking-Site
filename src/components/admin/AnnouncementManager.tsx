...
// Default "Affects from" to the next 15-minute mark so it lands on the DateTimeField time grid.
            ...
            <article key={announcement.id} className="rounded-xl bg-white p-4 ring-1 ring-gray-200 dark:bg-gray-900 dark:ring-gray-800">
              <div className="flex items-start justify-between gap-4">
                ...
                <div className="-mr-1 -mt-1 shrink-0">
                  <RowActionsMenu
                    label={`Actions for ${announcement.title}`}
                    actions={[
                      { label: 'Edit', icon: PencilIcon, onSelect: () => setEditing(announcement) },
                      {
                        label: 'Delete',
                        icon: TrashIcon,
                        danger: true,
...
function AnnouncementFormModal({
  ...
  const [startAt, setStartAt] = useState(announcement ? dateTimeLocal(announcement.startAt) : nextQuarterHourLocal())
  ...
  async function submit(event: React.FormEvent) {
    ...
    try {
      if (!startAt) {
        setError('Choose when the announcement takes effect ("Affects from").')
        return
      }
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={announcement ? 'Edit Announcement' : 'Add Announcement'} maxWidthClassName="max-w-2xl" variant="neutral">
      <form onSubmit={submit} className="scrollbar-thin flex max-h-[78vh] flex-col gap-6 overflow-y-auto pr-1">
        <div className="grid gap-4 md:grid-cols-[1fr_11rem]">
          <label className="flex flex-col gap-1.5 text-sm text-gray-700 dark:text-gray-300">Title *<input autoFocus value={title} onChange={(event) => setTitle(event.target.value)} required className="rounded-lg border border-gray-200 bg-white px-3 py-2 text-gray-900 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-100" /></label>
          <label className="flex flex-col gap-1.5 text-sm text-gray-700 dark:text-gray-300">Urgency *<select value={urgency} onChange={(event) => setUrgency(event.target.value as AdminAnnouncement['urgency'])} className="rounded-lg border border-gray-200 bg-white px-3 py-2 text-gray-900 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-100">{ANNOUNCEMENT_URGENCIES.map((value) => <option key={value} value={value}>{ANNOUNCEMENT_URGENCY_LABELS[value]}</option>)}</select></label>
        </div>
        <label className="flex flex-col gap-1.5 text-sm text-gray-700 dark:text-gray-300">Short message *<textarea value={message} onChange={(event) => setMessage(event.target.value)} required rows={4} className="rounded-lg border border-gray-200 bg-white px-3 py-2 text-gray-900 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-100" /></label>
        ...
        <label className="flex items-start gap-2 text-sm text-gray-700 dark:text-gray-300"><input type="checkbox" className="mt-0.5" checked={autoDisable} disabled={resourceIds.length === 0} onChange={(event) => setAutoDisable(event.target.checked)} /><span>Automatically take selected resources offline between &ldquo;Affects from&rdquo; and &ldquo;Affects until&rdquo;.</span></label>
