...
// ₱1,000 — front-desk cash/online top-ups have no fixed preset ceiling, only this floor.
export const ADMIN_TOPUP_MIN_CENTAVOS = 100_000

export function adminTopUpConfirmationText(amountCentavos: number, memberName: string): string {
  return `Add ${formatCentavos(amountCentavos)} in credit to ${memberName}`
}
