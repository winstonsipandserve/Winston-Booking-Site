...
- **Admin panel**: Bookings (list/detail/reschedule), Memberships (list/detail + approve/reject), Resources & Pricing (tabbed Courts/Simulators/Guest Fee — resources are edit (pricing) + disable/enable only, no create/delete; create/delete for pricing rows still disabled), Bulletin (full CRUD + image upload) — all live, gated by `(protected)/layout.tsx`. Not built: admin-side password reset, rate limiting, additional roles.
