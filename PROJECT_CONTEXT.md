...
- `BulletinCategory` is now a 7-value enum (added `Promotion` 2026-09-08), admin dropdown label shown in parentheses: `Renovation` ("Renovation"), `Closure` ("Facility Closure"), `Tournament` ("Tournament"), `Community` ("Community Event"), `General` ("General Announcement"), `FacilityMaintenance` ("Facility Maintenance"), `Promotion` ("Promotion").
- **`Promotion` category (added 2026-09-08)**: informational only — this bulletin type never touches checkout, PayMongo, or `PricingRule`; any actual discount is applied manually by an admin editing the existing Resources & Pricing screens. It carries three new nullable `Bulletin` fields, all display-only: `promoCode` (optional free-text code shown to customers, never validated/redeemed anywhere), `discountSummary` (required for this category — a short free-text description, e.g. "20% off" or "₱200 off bookings," deliberately a string rather than a structured percentage/amount pair since it's never computed against a real price), and `customerEligibility` (optional `BulletinCustomerEligibility` enum: `Everyone`/`MembersOnly`/`NewCustomers`/`ReturningCustomers`/`SpecificMembershipTier`, display only — not enforced at booking time). `Promotion` reuses existing fields rather than duplicating them: `eventStartAt`/`eventEndAt` (both required for this category, unlike every other category where `eventEndAt` is optional — an open-ended promotion doesn't make sense) is its validity window, `affectedFacility` is its applicable-facilities free text, and `ctaLabel`/`ctaUrl` is its booking link — no new columns for any of these. `Promotion` is the one category exempted from the `affectedFacility`/`impact`/`action` requirement every other category has (left optional, not hidden — an admin can still fill them in), and its `bookingImpact`/`customerActionType` (structured fields added 2026-09-08, required for every other category) are silently defaulted to `NoImpact`/`NoActionRequired` by the admin form rather than presented as blank required selects, since a promotion doesn't disrupt bookings. A published `Promotion` bulletin is deliberately excluded from `/book`'s `AnnouncementGate` (`GET /api/bulletin/gate-notices`'s query now filters `category: { not: 'Promotion' }`) since it shouldn't interrupt the booking flow the way a closure/maintenance notice does — it still appears on `/news` (no category filter there), where `discountSummary`/`promoCode` are folded into `NewsCard`'s existing Description block rather than getting their own labeled section, matching how no other category gets category-specific card treatment today.
- Optional metadata fields (added 2026-08-27, all nullable): `affectedFacility` (free-text, e.g. which court/bay/area is impacted), `impact` (free-text description of the effect on customers), `action` (free-text — what a customer should do in response, if anything), `eventStartAt`/`eventEndAt` (optional independent datetimes for a bulletin tied to a scheduled event or maintenance window — not required as a pair), `expiresAt` (optional date after which the bulletin is considered stale — enforced by `bulletinNotExpiredWhere()` in `src/lib/bulletin.ts`, applied by both `/news` and `GET /api/bulletin/gate-notices`'s query, so an expired bulletin is excluded from both public surfaces; the admin Bulletin list intentionally does not apply this filter, since admins need to see and manage expired bulletins too), `ctaLabel`/`ctaUrl` (optional pair, provided together or not at all, for an optional call-to-action link/button). A `priority` field (`BulletinPriority`: Normal/High) existed briefly but was removed 2026-08-31 — a client/business decision that announcements shouldn't carry a priority.
