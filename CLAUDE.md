...
| ORM | Prisma |
| Auth | Auth.js v5 — admin login decided and live (Credentials provider, JWT session, no adapter/DB session tables). Member-facing method is also decided (email/password) but not yet wired to Auth.js — `/login`/`/account` exist today only as a hardcoded-sample-data UI scaffold (sessionStorage-only session flag, no real backend); see Architecture Decisions → Customer & auth model |
