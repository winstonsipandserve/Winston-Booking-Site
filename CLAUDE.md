...
| ORM | Prisma |
| Auth | Auth.js v5 — admin login decided and live (Credentials provider, JWT session, no adapter/DB session tables); member-facing auth method still undecided and separate from this (see "Open / Not Yet Decided") |
...
- Member-facing auth method: email/password vs. magic link — still undecided. (Admin auth is separately decided and live: Auth.js Credentials provider, see Architecture Decisions.)
