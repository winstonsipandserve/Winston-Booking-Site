...
| ORM | Prisma |
| Auth | Auth.js v5 — both admin and member login are decided and live (two Credentials providers, JWT session, no adapter/DB session tables). `/account`'s displayed content is still hardcoded sample data pending real `Booking`/`Membership` wiring; see Architecture Decisions → Customer & auth model |
