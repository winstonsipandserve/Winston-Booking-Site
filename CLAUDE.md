...
- **Member-aware booking flow**: `/book` is reused for both anonymous and member bookings — not a separate route. `src/app/book/page.tsx` is now an async server component that mirrors `/account`'s `auth()` pattern: it reads the session, and if `session.user.role === 'member'`, fetches that `Customer` row and builds a `memberContext` (`name`/`email`/`phone`/`isActiveMember`) passed down to the renamed client orchestrator, `src/components/booking/BookingPageClient.tsx`. `isActiveMember(customerId)` was extracted out of `customer-resolution.ts` as a standalone export — it's now the single source of truth for "is this customer an active member" on both the pre-existing anonymous `PATCH /api/bookings/[id]` re-pricing path and the new session-aware `POST /api/bookings` hold-creation path, so the two can never drift. When `POST /api/bookings` sees a member session, it resolves `isMember` itself and attaches `customerId` directly at hold creation — a single-phase price, no `PATCH` round-trip — bypassing the two-phase-pricing flow described above entirely for that request (the two-phase flow remains exactly as-is for the anonymous path, including the case where an anonymous booker's email later turns out to belong to a member). Guest fee UI/backend are both gated on real active-membership status (`rateTier === 'member'` derived from `isActiveMember`), not merely on being logged in — a member whose membership has lapsed still sees non-member pricing and the guest-count control, matching how `resolveCustomer` already treated expired memberships.
...
- **Membership credit ledger**: `Membership.creditBalanceCentavos` is a cached running total, not the source of truth — a `MembershipCreditTransaction` model logs every event that changes a member's credit (activation, future POS spend, manual adjustments), each row immutable (`createdAt`-only, same pattern as `BookingReschedule`). The balance field exists for fast reads; the ledger exists for auditability and disputes. No spending events exist yet since POS isn't built — every row today is either an `activation` or `renewal` credit (`CreditTransactionReason` has both values) — but the ledger is in place so POS integration only adds a new `reason` value, not a new table or a balance-migration.
