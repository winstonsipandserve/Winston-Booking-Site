-- Add opaque, hashed browser capabilities for the anonymous booking flow.
ALTER TABLE "bookings"
ADD COLUMN "access_token_hash" TEXT,
ADD COLUMN "access_token_expires_at" TIMESTAMP(3);

-- Add a short-lived, HMAC-keyed rate-limit counter for credential and reset-email abuse.
...
-- RowLevelSecurity (Decision: Row Level Security â€” every new table enables RLS with deny-all policies in the same migration)
